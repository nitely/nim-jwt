...
proc pemDecoderLoop(
  ...
  var inobj = false
  while length > 0:
    var tlen = pemDecoderPush(pemCtx, unsafeAddr pem[offset], length.uint).int
    offset = offset + tlen
    length = length - tlen
...
proc bearSignRSPem*(
  ...
  assert(not s.isNil)
  if s(cast[ptr byte](hashOid), addr digest[0], hashLen.uint, addr pk, addr result[0]) !=
      1:
